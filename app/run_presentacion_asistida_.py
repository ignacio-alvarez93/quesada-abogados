...
def click_continuar(browser, session_dir):
    """
    Avanza desde Datos del extranjero/a a Datos del presentador.

    Solo corrige el click de continuar:
    - no usa btnConfirmarSup
    - evita el ID duplicado continuaPer buscando dentro de #tab-datos_personales
    - activa la pestaña de presentador si Mercurio ya la ha habilitado
    """
    print("[8] Avanzando a DATOS DEL PRESENTADOR")
    write_log(session_dir, "Click continuar correcto hacia Datos del presentador")
    ...
    (function(){
        ...
        function isPresentadorVisible() {
            const pre = document.getElementById('preNombrePresentador');
            if (!pre) return false;
            const panel = document.getElementById('tab-datos_presentador');
            if (!panel) return true;
            const st = window.getComputedStyle(panel);
            return st.display !== 'none' && !panel.className.includes('r-tabs-state-disabled');
        }

        // Botón correcto: el continuaPer situado dentro de Datos del extranjero/a.
        const scopedBtn = document.querySelector('#tab-datos_personales #continuaPer');
        if (scopedBtn) {
            fireClick(scopedBtn);
            return {ok:true, mode:'click_scoped_tab_datos_personales_continuaPer'};
        }

        // Fallback directo de Mercurio: continuar desde la pestaña personales.
        if (typeof continuarTab === 'function') {
            continuarTab('personales');
            return {ok:true, mode:'continuarTab_personales'};
        }

        // Si ya está habilitado, activar el tab presentador.
        const tabLink = document.getElementById('d-li-presentador') || document.querySelector('a[href="#tab-datos_presentador"]');
        if (tabLink) {
            fireClick(tabLink);
            return {ok:true, mode:'click_tab_presentador'};
        }

        return {
            ok:false,
            reason:'NO_CONTINUAR_A_PRESENTADOR',
            presentadorVisible:isPresentadorVisible(),
            continuaPerCount:document.querySelectorAll('#continuaPer').length
        };
    })();
    """

    result = js(browser, script)
    write_log(session_dir, f"AVANCE presentador -> {result}")
    time.sleep(1.5)

    try:
        wait_for_js(browser, "document.getElementById('preNombrePresentador')", timeout=8, interval=0.5)
    except Exception as exc:
        write_log(session_dir, f"WAIT presentador no confirmado: {exc}")
...
def click_continuar_presentador(browser, session_dir):
    """
    Pulsa CONTINUAR en Datos del presentador.
    Prioriza el botón #continuaPre.
    """
    print("[10] Continuar desde DATOS DEL PRESENTADOR")
    write_log(session_dir, "Click continuar presentador")
    ...
    (function(){
        ...
        const scoped = document.querySelector('#tab-datos_presentador #continuaPre');
        if (scoped) {
            fireClick(scoped);
            return {ok:true, mode:'click_tab_presentador_continuaPre'};
        }

        const btn = document.getElementById('continuaPre');
        if (btn) {
            fireClick(btn);
            return {ok:true, mode:'click_continuaPre'};
        }

        if (typeof continuarTab === 'function') {
            continuarTab('presentador');
            return {ok:true, mode:'continuarTab_presentador'};
        }

        return {
            ok:false,
            reason:'NO_CONTINUA_PRE',
            ids:Array.from(document.querySelectorAll('[id]')).map(x => x.id).slice(0, 250)
        };
    })();
    """

    result = js(browser, script)
    write_log(session_dir, f"CLICK continuar presentador -> {result}")
    time.sleep(1.2)
    return result
