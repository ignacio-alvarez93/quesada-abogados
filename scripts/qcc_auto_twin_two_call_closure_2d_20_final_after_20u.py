"""WO 2D-20 FINAL (post-2D-20U): fixed-point two-call materialization
closure.

Re-attempt of the authorized two-call closure (CALL 1 pinned to the
original base, materializing new states; CALL 2 pinned to CALL 1's own
revision, materializing newly eligible transitions) now that 2D-20U
has added the governed CAUSAL_BASELINE_FALLBACK branch to
_new_state_navigation_source(). The prior attempt blocked with
PASS2_BLOCKED_CAUSAL_LAST_EVIDENCE_INCOMPLETE because TITULAR's
recorded last_capture_id (20260912_060228_192874_aa602bac) is missing
page.mhtml/screenshot_viewport.png, and the pre-existing
CAUSAL_EQUIVALENT search cannot see past the pre-2D-20H generic
fingerprint cached on every capture's state_observation.json.

This script:

1. Verifies no invariant has drifted since the end of 2D-20U (revision
   count, observation state) before writing anything.
...
3. Executes CALL 1 (PINNED to matrev-1586425db7664a40678bfa82),
   independently inspects the resulting revision from disk, and stops
   before any further write if any invariant fails.
...
of the three known non-conforming revisions. Does not modify source,
...
# Initial invariants asserted at the end of 2D-20U.
# _mercurio_revision_dirs() lists children of materialized/mercurio/
# only (Path.iterdir() never includes the directory itself) -- 191
# revisions, matching `find data/.../mercurio -maxdepth 1` (192)
# minus the mercurio/ root entry itself.
EXPECTED_INITIAL_REVISION_DIR_COUNT = 191
...
def main() -> int:
    ...
    print("=== INITIAL INVARIANT CHECK (must match end of 2D-20U) ===")
    print(
    ...
    print("initial invariants OK -- unchanged since end of 2D-20U.\n")
    ...
    print("TITULAR source_reason: CAUSAL_BASELINE_FALLBACK (proven pre-flight)")
