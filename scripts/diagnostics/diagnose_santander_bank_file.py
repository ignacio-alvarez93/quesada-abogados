...
from backend.services.economic_reconciliation.bank_santander_parser_service import (  # noqa: E402
    cents_to_eur,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Diagnóstico de exportación bancaria Santander."
    )
    ...
    report = diagnose_santander_bank_file(args.file)

    print("")
    print("== Diagnóstico Banco Santander ==")
    print(f"Archivo: {report.source_file}")
    print(f"Formato: {report.detected_format}")
    print(f"SHA256: {report.file_sha256}")
    print(f"Export timestamp: {report.export_timestamp or '-'}")
    ...
    print("")
    print("Política:")
    print("  El banco se importa como movimiento bancario bruto. No crea cobros, facturas ni vínculos automáticos.")
