...
# processing regression coverage (full select -> batch pipeline).
# ---------------------------------------------------------------------------


def test_repeated_select_and_batch_over_unchanged_snapshot_is_idempotent(tmp_path):
    capture_root = tmp_path / "site_architecture"
    _write_capture(capture_root, capture_id="cap-A", elements=[_element("#a")])
    _write_capture(
        capture_root, capture_id="cap-B", elements=[_element("#a"), _element("#b")]
    )
    ...
    def _run_cycle():
        selection = select_contract_watcher_persisted_watch_requests(
            store, confirmation_policy=_policy(), capture_root=capture_root
        )
        return run_contract_watcher_persisted_cycle_batch(
            selection["selected"],
            evidence_store=evidence_store,
            history_store=history_store,
        )
    ...
    # Simulates a restarted worker (or an overlapping scheduler tick)
    # re-deriving and reprocessing the exact same, unchanged AUTO TWIN
    ...
    contract_key = first_run["results"][0]["contract_key"]
...
def test_new_capture_after_previous_cycle_is_distinguishable_from_replay(tmp_path):
    capture_root = tmp_path / "site_architecture"
    _write_capture(capture_root, capture_id="cap-A", elements=[_element("#a")])
    _write_capture(
        capture_root, capture_id="cap-B", elements=[_element("#a"), _element("#b")]
    ...
    # A genuinely new capture arrives after the previous cycle already
    # ran -- AUTO TWIN's own bookkeeping advances last_capture_id, never
    # this module's concern to detect on its own.
    _write_capture(
        capture_root,
        capture_id="cap-C",
    ...
    assert second_run["results"][0]["contract_key"] == contract_key
    assert second_run["results"][0]["outcome"] == (
        ContractWatcherCycleOutcome.OBSERVATION_REGISTERED.value
    )
    assert (
        second_run["results"][0]["evidence_id"]
        != first_run["results"][0]["evidence_id"]
    )
...
def test_boundary_intermediate_capture_superseded_before_processing_is_skipped(
    tmp_path,
):
    """Documents the one restart-safety boundary this slice cannot close.

    The selector (1E) derives its request purely from AUTO TWIN's CURRENT
    ``(baseline_capture_id, last_capture_id)`` pointers -- there is no
    durable, per-contract queue of every intermediate capture that was
    ever current between two processing passes. If a capture becomes
    "current" and is then superseded by a newer one before any
    select+batch pass ever observes it (e.g. a worker was down across
    both captures), that intermediate capture is safely, silently never
    compared against the baseline -- it is neither duplicated nor
    corrupted, but it is also never durably evidenced on its own.
    Closing this would require a durable per-contract processed-capture
    cursor/queue, which is new shared schema/infrastructure explicitly
    ...
    # Two captures land while no worker ever runs a select+batch pass in
    # between -- cap-B becomes "current", then is itself superseded by
    # cap-C before anything ever reads/compares it.
    _observe(
        store,
        site,
        capture_id="cap-B",
    ...
    # Only one request is ever derived: baseline (cap-A) directly against
    # the now-current last_capture_id (cap-C). cap-B is neither selected
    # nor referenced anywhere.
    assert len(selection["selected"]) == 1
    assert selection["selected"][0].observation_capture.capture_id == "cap-C"
    ...
    contract_key = result["results"][0]["contract_key"]

    status = get_contract_watcher_lifecycle_status(
        contract_key, history_store=history_store
    )

    # Exactly one durable observation exists for this contract, and it
    # only ever references the baseline and the final, superseding
    # capture -- cap-B was never independently evidenced.
    assert status["history_length"] == 1
    assert result["results"][0]["baseline_capture_id"] == "cap-A"
