...
class ReagrupacionVisaFingerprintsEndToEndTest(
    unittest.TestCase
):
    def setUp(self):
    ...
    def test_reagrupacion_favorable_creates_and_accepts_visa_proposal(
        ...
        visa_evaluation = visa_event_result[
            "derivation_evaluation"
        ]

        self.assertTrue(visa_evaluation["ok"])
        self.assertEqual(
            visa_evaluation["rules_evaluated"],
            1,
        )
        self.assertEqual(
            len(visa_evaluation["proposals"]),
            1,
        )

        fingerprint_proposal = (
            visa_evaluation["proposals"][0]
        )

        self.assertTrue(
            fingerprint_proposal["created"]
        )
        self.assertEqual(
            fingerprint_proposal["regla_codigo"],
            (
                "VISADO_REAGRUPACION_"
                "CONCEDIDO_A_HUELLAS"
            ),
        )

        fingerprint_proposal_id = (
            fingerprint_proposal[
                "proposal"
            ]["id"]
        )
        ...
        self.assertTrue(
            fingerprint_acceptance["created"]
        )
        ...
        self.assertEqual(
            fingerprints["cliente_id"],
            1,
        )
        self.assertEqual(
            fingerprints["tipo_codigo"],
            "TOMA_HUELLAS",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            int(relations[0]["expediente_origen_id"]),
            1000,
        )
        self.assertEqual(
            int(relations[0]["expediente_destino_id"]),
            destination_id,
        )
        self.assertEqual(
            int(relations[1]["expediente_origen_id"]),
            destination_id,
        )
        self.assertEqual(
            int(relations[1]["expediente_destino_id"]),
            fingerprints_id,
        )
        ...
        origin_levels = {
            ...
            for node in chain_from_origin["nodes"]
        }

        visa_levels = {
            int(node["id"]): int(node["nivel"])
            for node in chain_from_visa["nodes"]
        }

        fingerprints_levels = {
            int(node["id"]): int(node["nivel"])
            for node
            in chain_from_fingerprints["nodes"]
        }
