...
def inject_navigation_runtime_adapter(
    ...
    *,
    ...
    script = (
        ...
        "  };\n"
        "\n"
        # QCC_ONCLICK_STRUCTURAL_RUNTIME_MATCH_V1 (Twin adapter)
        #
        # Mirrors the governed extension fallback: exact CSS
        # matching/closest is always tried first; the structural
        # ONCLICK fallback (including its privacy-safe positional
        # disambiguation suffix, :qcc-nth-onclick(N)) only ever
        # activates when the exact path found nothing, and never
        # reads/transports the physical literal argument.
        "  const ONCLICK_SAFE_HANDLER_RE =\n"
        "    /^(?:return\\s+)?(?:window\\.)?[A-Za-z_$][A-Za-z0-9_$]*"
        "\\(\\s*(?:(?:'[A-Za-z0-9_ .:/-]{0,128}'"
        '|"[A-Za-z0-9_ .:/-]{0,128}"'
        "|-?[0-9]+(?:\\.[0-9]+)?|true|false|null)"
        "(?:\\s*,\\s*(?:'[A-Za-z0-9_ .:/-]{0,128}'"
        '|"[A-Za-z0-9_ .:/-]{0,128}"'
        "|-?[0-9]+(?:\\.[0-9]+)?|true|false|null))*)?\\s*\\)\\s*;?$/;\n"
        "\n"
        ...
        "    /^([A-Za-z][A-Za-z0-9]*)\\[onclick=\""
        "((?:return\\s+)?(?:window\\.)?[A-Za-z_$][A-Za-z0-9_$]*"
        '\\(\\)\\s*;?)"\\]'
        "(?::qcc-nth-onclick\\((\\d+)\\))?$/;\n"
        "\n"
        "  const qccOnclickStructuralSignature = (rawValue) => {\n"
        "    const value = String(rawValue || \"\");\n"
        "\n"
        "    if (!ONCLICK_SAFE_HANDLER_RE.test(value)) return null;\n"
        "\n"
        "    const nameMatch = value.match(ONCLICK_HANDLER_NAME_RE);\n"
        "\n"
        ...
        "    }\n"
        "\n"
        "    const trailingSemicolon = value.trim().endsWith(\";\");\n"
        "\n"
        "    return nameMatch[0] + \")\" + (trailingSemicolon ? \";\" : \"\");\n"
        "  };\n"
        "\n"
        "  const qccParseOnclickStructuralSelector = (selector) => {\n"
        "    const match = ONCLICK_SELECTOR_RE.exec(selector);\n"
        "\n"
        ...
        "  };\n"
        "\n"
        "  const qccOnclickStructuralPosition = (node, parsed) => {\n"
        "    let candidates;\n"
        "\n"
        "    try {\n"
        "      candidates = document.querySelectorAll(parsed.tag);\n"
        "    } catch (_) {\n"
        "      return null;\n"
        "    }\n"
        "\n"
        ...
        "\n"
        "      try {\n"
        "        rawOnclick = candidate.getAttribute(\"onclick\");\n"
        "      } catch (_) {\n"
        "        continue;\n"
        "      }\n"
        "\n"
        "      if (!rawOnclick) continue;\n"
        "\n"
        "      const signature = qccOnclickStructuralSignature(rawOnclick);\n"
        "\n"
        "      if (signature === null || signature !== parsed.signature) {\n"
        "        continue;\n"
        "      }\n"
        "\n"
        "      rank += 1;\n"
        "\n"
        "      if (candidate === node) return rank;\n"
        "    }\n"
        "\n"
        "    return null;\n"
        "  };\n"
        "\n"
        "  const qccMatchesOnclickStructural = (node, parsed) => {\n"
        "    if (\n"
        "      !node ||\n"
        "      node.nodeType !== 1 ||\n"
        ...
        "    }\n"
        "\n"
        "    if (String(node.tagName || \"\").toLowerCase() !== parsed.tag) {\n"
        "      return false;\n"
        "    }\n"
        "\n"
        "    let rawOnclick;\n"
        "\n"
        "    try {\n"
        "      rawOnclick = node.getAttribute(\"onclick\");\n"
        "    } catch (_) {\n"
        "      return false;\n"
        "    }\n"
        "\n"
        "    if (!rawOnclick) return false;\n"
        "\n"
        "    const signature = qccOnclickStructuralSignature(rawOnclick);\n"
        "\n"
        "    if (signature === null || signature !== parsed.signature) {\n"
        ...
        "    }\n"
        "\n"
        "    if (parsed.position === null) return true;\n"
        "\n"
        "    const position = qccOnclickStructuralPosition(node, parsed);\n"
        "\n"
        "    return position !== null && position === parsed.position;\n"
        "  };\n"
        "\n"
        ...
        "      } catch (_) {\n"
        "        // Not native CSS: may be a governed structural\n"
        "        // fallback form. Continue below.\n"
        "      }\n"
        "    }\n"
        "\n"
        "    const parsedOnclick = qccParseOnclickStructuralSelector(selector);\n"
        "\n"
        "    if (!parsedOnclick) return false;\n"
        "\n"
        "    for (const node of nodes) {\n"
        "      let cursor = node instanceof Element ? node : null;\n"
        "\n"
        "      while (cursor) {\n"
        "        if (qccMatchesOnclickStructural(cursor, parsedOnclick)) {\n"
        "          return true;\n"
        "        }\n"
        "\n"
